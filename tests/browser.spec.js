...
test.beforeAll(async()=>{
 server=http.createServer(async(req,res)=>{
  ...
  if(req.url==='/download'){
   ...
   let sent=start;const timer=setInterval(()=>{const len=Math.min(65536,total-sent);if(len>0){res.write(Buffer.alloc(len,67));sent+=len;}if(sent>=total){clearInterval(timer);res.end();}},100);res.on('close',()=>clearInterval(timer));return;
  }
...
test('downloads show actual progress and support runtime pause/resume',async()=>{
 ...
 await call('resume-download',d.id);await expect.poll(async()=>(await state()).downloads[0].state,{timeout:10000,message:'wait for resumed download completion'}).toBe('completed');expect(fs.statSync(savePath).size).toBe(2097152);
